export function envLookup (name) {
  if (typeof Deno !== "undefined") return Deno.env.get(name);
  else if (typeof process?.env !== "undefined") return process.env[name];
  return "";
}

export function report (msg,items,state) {
  if (state === undefined) state = true;
  if (!msg) msg = "Unspecified message.";
  if (!items[items.length-1]) items[0] = 0;
  let temp = items[items.length-1];
  items.pop();
  items[items.length] = msg;
  items[items.length] = state;
  if (items[items.length-1] !== undefined) items[items.length] = temp + 1;
}

export function tabulateList(items) {
  let nth = items[items.length-1],
  cur = nth,
  itR8 = 1,
  list = "",
  order;
  for (;cur;--cur) {
    order = nth-cur+1;
    list += `${order}. ${items[itR8-1]}</br>`;
    itR8 = itR8 + 2;
  }
  return list.substring(0,list.length);
}

export function checkValues(items,type) {
  if (type === undefined) type = true;
  const nth = items[items.length-1];
  let itR8 = nth,
  cur,
  ndx;
  for (;itR8;--itR8) {
    cur = nth - itR8;
    ndx = (cur * 2) + 1;
    if (items[ndx] === undefined) continue;
    if (items[ndx] === type) continue;
    else return false;
  }
  return true;
}
...
export function buildEmailHtml (fields) {
  ...
  keysTitles = [];
  let contents = "<h1>New Form Submission</h1>",
  nth = keysLength,
  itR8 = keys?.[0]?.length,
  len = itR8,
  cur = itR8,
  ...
  for (;nth;--itR8) {
    if (!itR8) {
      keysTitles[keysLength - nth] = keySmither;
      keySmither = "";
      contents = contents + `<p><strong>${keysTitles?.[keysLength - nth]}:</strong>${JSON.stringify(fields?.[keys?.[keysLength - nth]]) || "N/A"}</p>`
      --nth;
      if (!nth) break;
      len = keys[keysLength - nth].length;
      itR8 = len;
    }
    cur = len - itR8;
    char = keys[keysLength - nth].charCodeAt(cur);
    prevChar = keys[keysLength - nth].charCodeAt(cur - 1);
    if (len === itR8) {
      if (char < 123 && char > 96) {
        keySmither = keySmither + String.fromCharCode(char - 32);
        continue;
      }
    }
    if (prevChar === 95 || prevChar === 45) {
      if (char < 123 && char > 96) {
        keySmither = keySmither + String.fromCharCode(char - 32);
        continue;
      }
    ...
    keySmither = keySmither + String.fromCharCode(char);
  }
  return contents;
}

export async function webhookRequest (id) {
  try {
    await axios.get(`${envLookup("WEBHOOK_URL")}/hooks/${id}`,
    {
      headers: {
        "X-Webhook-Token": envLookup("WEBHOOK_TOKEN"),
      }
    }).then((resp) => {
      print(`Response: ${JSON.stringify(resp.data)}`,1);
    });
