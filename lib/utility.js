...
export function tabulateList(items) {
  ...
  for (;cur;--cur) {
    ...
    //if (items[itR8]) states[1].push(itR8);
    //else states[0].push(itR8);
...
export function checkValues(items,values,type) {
  if (type === undefined) type = true;
  let i = values.length;
  for (;i;--i) {
    if (items[values[i - 1]] === undefined) continue;
    if (items[values[i - 1]] === type) continue;
    else return false;
  }
  return true;
}
